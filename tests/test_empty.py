...
def test_empty(shape, dtype):
    ...
    if flag_gems.vendor_name == "cambricon":
        # torch.empty(..., dtype=..., device=...) dispatches to aten::empty.memory_format,
        # not the ("empty", empty) entry registered in _FULL_CONFIG.
        res_out = flag_gems.empty(*shape, dtype=dtype, device=flag_gems.device)
    else:
...
def test_empty_default_dtype():
    ...
    expected_dev = "cpu" if cfg.TO_CPU else device
    if flag_gems.vendor_name == "cambricon":
        # torch.empty(..., device=...) dispatches to aten::empty.memory_format,
        # not the ("empty", empty) entry registered in _FULL_CONFIG.
        res_out = flag_gems.empty(10, 20, device=flag_gems.device)
    else:
        with flag_gems.use_gems():
